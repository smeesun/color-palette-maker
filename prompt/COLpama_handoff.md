...
**重要**: `preset_entry_tool.py` は `main.py` と**同じ階層**に置く必要がある。`sqlite3.connect("colpama.db")` が相対パスで呼ばれているため、実行時のカレントディレクトリを基準に `colpama.db` を探すことになり、サブフォルダに分けるとパス指定を書き換える必要が出てくるため。
...
5. **カテゴリのタグ選択のUX**: 本体側(main.py)のSingleSelectGroupは単一選択のまま。preset_entry_tool.py側は複数選択・トグル式に変更済みだが、本体側も同様に複数選択にすべきかは要検討・未確定
...
## 6. カテゴリ構想(確定事項・追記 2026-08-06)

カテゴリセクションの構成について、以下の通り確定した。現状のコード(`main.py`の`tone_group`/`era_group`/`season_group`、`preset_entry_tool.py`の同名タグ群)はこの確定前の暫定構成(トーン/年代/季節の3本柱)であり、本節の内容はまだ実装には反映されていない次の仕様。

### 4本柱

「イメージ」「季節」「年代」「地域」の4カテゴリで確定。それぞれ性質の異なる切り口(印象語・パーソナルカラー由来・トレンド文化・地理文化)を担う。現行の「トーン」は「イメージ」に置き換わる位置づけ。
...
このカテゴリは今後も新しい単語が追加され続ける前提(固定リストではなく成長し続けるカテゴリ)。データ登録ツール(`preset_entry_tool.py`)の「新規」追加機能と連動しており、新しい単語が登録されると本体(`main.py`)側にも反映される設計。

### 季節・年代(固定リストのまま)

- 季節: 春/夏/秋/冬
- 年代: 70's/80's/90's/00's

### 地域(7つで確定、並び順は目安)

南米・メキシコ → 北欧 → 地中海 → アフリカン → 中東・モロッコ → 東アジア → 南国・トロピカル

カルチャー・歴史が色に表れる文化圏をテーマにした配色カテゴリで、地理的な正確さより「配色文化としての独自性」を優先して選定。
